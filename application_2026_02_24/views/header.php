...
<div style="margin:0 auto; width:100%; height:50px;; overflow:hidden; background: url(<?php echo base_url(); ?>uploads/company_photo/01.jpg)  no-repeat;">
<div style=" width:60%; font-weight: 900; text-align:center; font-family:'Comic Sans MS';float:left;">
    ...
    <img  height="40" src="<?php echo base_url(); ?>images/<?php echo $company_logo; ?>" />
	</div>
	<div style=" height:auto; width:60%; overflow:hidden; font-size:25px; float:right; text-align:right;">
	<?php 
	$session_unit_id = $this->common_model->get_session_unit_id_name();
	if($session_unit_id != 0){
		echo $this->db->where("unit_id",$session_unit_id)->get('pr_units')->row()->unit_name;
	}
	//else{ echo $company_name_english = $this->common_model->company_information("company_name_english");}
	?>
	</div>
</div>
