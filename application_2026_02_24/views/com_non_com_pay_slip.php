...
<title>Actual Pay Slip Mix</title>
<style type="text/css">
<!--
...
foreach($values as $rows)
{
 ...
 $this->load->view('head_bangla'); ?>
				...
				<?php 
				 $staff = $this->Grid_model->staff_id_collect($rows["emp_id"]);
					...
					if($rows['ot_entitle']==1 && $staff==1){
						...
						$eot_hour = $rows["eot_hour"];

					}elseif($rows['ot_entitle']==0 && $staff==0){
						//echo "hey";
						$ot_hour = $rows["ot_hour"];
						$eot_hour = $rows["eot_hour"];
					}
					...
					// echo $total_days = $rows["ot_hour"] + $rows["eot_hour"];   
					echo $total_days = $ot_hour + $eot_hour;   
				?>
			</font> 
			</td>
		</tr>
  ...
  <!--add-->
	...
	<tr>
	  	...
	  	<td >:</td>
		<td width="59" align="right"> <font style="font-family: SutonnyMJ; font-size:14px;"> <?php echo $total_days = $rows["abs_deduct_cnonc_mix"];    ?> </font></td>
	</tr>
	<!--<tr>
	  	<td width="152">লেট </td>
	  	<td >:</td>
	...
	<tr>
	  	...
	  	<td >:</td>
		<td width="59" align="right"> <font style="font-family: SutonnyMJ; font-size:14px;"> <?php echo $total_days = $rows["total_deduction_cnonc_mix"];   ?> </font></td>
	</tr>
   ...
   <div style="border-bottom:1px solid #000000; line-height: 25px;"><table width="295" cellspacing="0" cellpadding="0">
    <tr>
	...
	<div style="border-bottom:1px solid #000000; line-height: 25px;"><table width="298" cellspacing="0" cellpadding="0">
    <tr>
      <td width="210"><span style="position:relative; left:5px; font-weight:bold;"> (ঘ) অতিরিক্ত কাজ</span></td>
	   <td >:</td>
	  <td width="45" style="padding-right: 12px;" align="right"> 
	  <?php 

			if($rows['ot_entitle']==1 && $staff==1){
				$ot_amount =  $rows["ot_amount"];
				$eot_amount = $rows["eot_amount"];
				$net_pay_cnonc_mix = $rows["net_pay_cnonc_mix"] + $rows["eot_amount"];
		...
		?>
	  <span style="font-family: SutonnyMJ; font-size:14px;" > <?php echo $totamt = $ot_amount + $eot_amount; ?> </span>
	  </td>
    </tr></table></div>
    ...
    <tr>
      ...
      <td >:</td>
	  <td width="78" style="padding-right: 9px;" align="right"> <font style="font-family: SutonnyMJ; font-size:14px;"> <?php echo $total_days = $net_pay_cnonc_mix;   ?> </font></td>
    </tr></table></div>
  </div>
 ...
 <?php $this->load->view('head_bangla'); ?>
  ...
  <div style="width:312px; height:auto; margin-top:12px; line-height: 14px; overflow:hidden; border:1px solid #000000;">
