...
<?php
	...
	for($p=0; $p<=$per_page_row;$p++)
	{
		...
		if (!empty($value[$k]->holiday_alo_count) && !empty($value[$k]->weekend_alo_count)) {
			$alc = ($value[$k]->holiday_allowance_rate + $value[$k]->weekend_allowance_rate ) /2;
		} else if (!empty($value[$k]->holiday_alo_count)) {
			$alc = $value[$k]->holiday_allowance_rate;
		} else if ($value[$k]->weekend_alo_count) {
			$alc = $value[$k]->weekend_allowance_rate;
		} else {
			$alc = 0;
		}
				
		echo "<td>";
		echo $holiday_alo_count  = $value[$k]->holiday_alo_count + $value[$k]->weekend_alo_count;
		echo "</td>";
		...
		$holiday_allowance =  $value[$k]->holiday_allowance + $value[$k]->weekend_allowance;
